 ...
 */
@WebServlet(name = "LabelMakerVN", urlPatterns
        = {
            "/vn/LabelMaker"
        })
public class LabelMakerVN extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        udb.addUser("[VN];" + IPDetector.getClientIpAddr(request));

        Labels labels = new Labels("vietnamese");
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            out.println("<div class=\"header\">Label Maker&#8480;"
                    ...
                    + "<img src=\"/EnterpriseApps/images/vietnam_flag.png\" alt=\"Vietnamese\" class=\"langimg\"/>"
                    + "</div>"
                    + "</div>");
            ...
            out.println("<form method=\"POST\" action=\"/EnterpriseApps/LabelMaker/PreviewPDF\" accept-charset=\"UTF-8\" name=\"FForm\" target=\"pdfFrame\">"
                    ...
                    + "<tr>"
                    + "<td>"
                    + "<div class=\"label\" title=\"Hãy điền tên mặt hàng\">"
                    + labels.getTitle()
                    + "</div>"
                    + "</td>"
                    + "<td>"
